...
public class ColorTransfusion_01 : MonoBehaviour
{
  /// <summary>
  ...
  /// <returns>Новый цвет от процентного смешивания двух цветов </returns>
    ...
    {
        return new Color(Random.Range(0, 1001)/1000f, Random.Range(0, 1001) / 1000f, Random.Range(0, 1001) / 1000f);
    }
    ...
    {
        float r = required.r * perсent + current.r * (1f - perсent);
        float g = required.g * perсent + current.g * (1f - perсent);
        float b = required.b * perсent + current.b * (1f - perсent);
